...
@section('contenido')
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2">
            ...
            <div>
                <p class="font-bold"> {{ $post->user->username}}</p>    {{--el user es el nombre de la relacion que definimos en el modelo Post--}}
                <p class="text-sm text-gray-500">
                    {{ $post->created_at->diffForHumans() }} {{--el created_at es el nombre de la columna en la base de datos--}}
                </p>
                <p class="mt-5">
                    {{ $post->descripcion }}
                </p>
            ...
            @auth
                @if($post->user_id === auth()->user()->id) {{--verificamos si el id del post es igual al id del usuario autenticado--}}
                    <form method="POST" action="{{ route('posts.destroy', $post) }}">

                        @method('DELETE') {{-- metodo spoofing para eliminar porque el navegador no soporta DELETE sólo "get" y "post"--}}
        ...
        <div class="md:w-1/2 p-5">
            ...
            <div class="shadow bg-white p-5 mb-5">
                ...
                <form action="{{ route('comentarios.store', ['post' => $post, 'user' => $user]) }}" method="POST">
                    @csrf
                    <div class="mb-5">
                        <label for="comentario" class="mb-2 block uppercase text-gray-500 font-bold">Añande un Comentario</label>
